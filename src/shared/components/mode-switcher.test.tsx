...
describe("ModeSwitcher", () => {
	...
	it('renders the "Immersive" tab as selected on initial render', () => {
		render(<ModeSwitcher />);
		const immersiveTab = screen.getByRole("tab", { name: "Immersive" });
		expect(immersiveTab).toHaveAttribute("aria-selected", "true");
	});

	it('renders the "Recruiter" tab as selected when pathname is "/recruiter"', () => {
		vi.mocked(usePathname).mockReturnValue("/recruiter");
		render(<ModeSwitcher />);
		const recruiterTab = screen.getByRole("tab", { name: "Recruiter" });
		expect(recruiterTab).toHaveAttribute("aria-selected", "true");
	});

	it('updates the store and navigates to "/recruiter" when the "Recruiter" tab is clicked', async () => {
	...
	it('navigates to "/" when the "Immersive" tab is clicked from "/recruiter"', async () => {
		vi.mocked(usePathname).mockReturnValue("/recruiter");
		const user = userEvent.setup();
		render(<ModeSwitcher />);
